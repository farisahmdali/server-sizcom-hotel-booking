...
module.exports = {
  ...
  roomdetails: async (req, res) => {
    try {
      const data = await getDb()
        .collection("booking")
        .find({ payed: true, room: req.query.type })
        .toArray();
      let bookedDate = [];
      let count = {};
  ...
  search: async (req, res) => {
    try {
      ...
      const data = await getDb()
        .collection("booking")
        .find({ payed: true, dates: { $in: allDates } })
        .toArray();
      let bookedDatePool = [];
      let bookedDateApart = [];
      let bookedDateSmall = [];
  ...
  login: (req, res) => {
    try {
      ...
      if (
        ...
        token = jwt.sign(req.query.username, process.env.KEY);
  ...
  data:async(req,res)=>{
    try {
      const data =await getDb().collection("booking").find({payed:true}).toArray()
      console.log(data)
  ...
  delete:(_id)=>{
    getDb().collection("booking").deleteOne({_id:new ObjectId(_id)})
  }
};
